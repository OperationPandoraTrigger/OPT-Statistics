...
# Get all players
mariadb -u ${DB_USER} -p${DB_PASS} ${DB_NAME} --skip-column-names --silent --execute="SELECT SteamID64, Nickname, DATE_FORMAT(Seenfirst, '%d.%m.%Y') FROM Players WHERE SteamID64 != 0;" > ${TEMPDIR}/players.tmp
...
do
	...
	PARTICIPATIONS=${player[20]}
        ...
        echo Stats for ${NICK}: kills: $KILLS - rev: $REVIVES - flags: $FLAGS - trapo: $TRAPO - vk: $VEHICLEKILLS - participations: $PARTICIPATIONS
